...
<html>
	<head>
		...
		<title>Visualizar Clínica</title>
	</head>
	<body>
		...
		<div class="nav" role="navigation">
			<ul>
				...
				<li><g:link class="list" action="listagem">Lista de Clínicas</g:link></li>
				<sec:access expression="hasRole('ROLE_ADMINISTRADOR')">
				<li><g:link class="create" action="cadastrar">Cadastrar Clínica</g:link></li>
				</sec:access>
			</ul>
		</div>
		<div id="show-clinica" class="content scaffold-show" role="main">
			<h1>Visualizar clinica</h1>
			...
			<ol class="property-list clinica">
				...
				<sec:access expression="hasRole('ROLE_ADMINISTRADOR')">
				<g:if test="${clinicaInstance?.limiteConsultas}">
				<li class="fieldcontain">
					<span id="limiteConsultas-label" class="property-label"><g:message code="clinica.limiteConsultas.label" default="Limite Consultas" /></span>
					
						<span class="property-value" aria-labelledby="limiteConsultas-label"><g:fieldValue bean="${clinicaInstance}" field="limiteConsultas"/></span>
					
				</li>
				</g:if>
				</sec:access>
			
				<g:if test="${clinicaInstance?.especialidades_clinica}">
				<li class="fieldcontain">
					<span id="especialidades_clinica-label" class="property-label"><g:message code="clinica.especialidades_clinica.label" default="Especialidadesclinica" /></span>
					
						<g:each in="${clinicaInstance.especialidades_clinica}" var="e">
						<span class="property-value" aria-labelledby="especialidades_clinica-label"><g:link controller="especialidade" action="visualizar" id="${e.id}">${e?.encodeAsHTML()}</g:link></span>
						</g:each>
				...
				<li class="fieldcontain">
					<span id="medicos_clinica-label" class="property-label"><g:message code="clinica.medicos_clinica.label" default="Medicos da Clínica" /></span>
						<g:if test="${clinicaInstance?.medicos_clinica}">	
						<g:each in="${clinicaInstance.medicos_clinica}" var="m">
						<span class="property-value" aria-labelledby="medicos_clinica-label"><g:link controller="medico" action="visualizar" id="${m.id}">${m?.nome?.encodeAsHTML()}</g:link></span>
						</g:each>
						</g:if>
						<sec:access expression="hasRole('ROLE_ADMINISTRADOR')">
						<span class="property-value" aria-labelledby="medicos_clinica-label"><g:link controller="medico" action="cadastrar" params="['clinica.id': clinicaInstance?.id]">Cadastrar Médico</g:link></span>
						</sec:access>
				</li>
