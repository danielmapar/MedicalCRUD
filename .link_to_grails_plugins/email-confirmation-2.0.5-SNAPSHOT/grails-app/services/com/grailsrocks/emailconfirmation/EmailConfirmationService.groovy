...
class EmailConfirmationService implements ApplicationContextAware {
	...
	def makeURL(token) {
		...
		def serverURL = grailsApplication.config.grails.serverURL ?: 'http://localhost:6060/'+grailsApplication.metadata.'app.name'
		"${serverURL}/confirmacao/${token.encodeAsURL()}"
	}
	...
	def sendConfirmation(Map args) {
		...
		try {
			...
			sendMail {
				...
				body( view: viewName,
					  plugin: pluginName,
					  model:[fromAddress: args.from, uri: binding.uri])
			}
